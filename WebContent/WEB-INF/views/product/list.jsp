...
<title>Volta | ${category.name}</title>
			...
			<div class="col-lg-12">
				<div class="pageTitleContent">
					...
					<div class="breadcrumbs">
						<a href="home/index.htm">trang chủ</a> &nbsp;/ &nbsp;<a>${category.name}</a>
					</div>
				</div>
			</div>
		</div>
...
<c:choose>
													<c:when test="${p.discount!=0}">
														<p class="prices">
															<del><f:formatNumber> ${p.unitPrice}</f:formatNumber></del>
															&nbsp;<f:formatNumber>${p.unitPrice*(1-p.discount)}</f:formatNumber> 
														</p>
													</c:when>
													<c:otherwise>
														<p class="prices"><f:formatNumber>${p.unitPrice}</f:formatNumber> </p>
													</c:otherwise>
												</c:choose>
								</div>
							</div>
